...
b2Body* Frog::CreateMonster(b2World* world, Vec2 position, Layer* layer) {
	...
	m_MonsterSprite = Sprite::createWithTexture(m_texture, Rect(0, 0, 35, 32));
...
Sprite* Frog::AnimationState(_moveState state, Layer* layer) {
	int nSheetmx;
	Size size;
	std::string sPath;
	...
	case MS_LEFT:
		...
		size.width = 35;
		size.height = 32;
		sPath = "Images/Monsters/Frog_Jump.png";
		break;
	case MS_RIGHT:
		nSheetmx = 2;
		size.width = 35;
	...
	case MS_STOP:
		...
		size.width = 35;
		size.height = 32;
		sPath = "Images/Monsters/Frog_idle.png";
		break;
	case MS_LEFTJUMP:
		nSheetmx = 1;
		size.width = 35;
		size.height = 32;
		sPath = "Images/Monsters/Frog_jumpUp.png";
		break;
	case MS_RIGHTJUMP:
		nSheetmx = 1;
		size.width = 35;
	...
	case MS_PAIN:
		...
		size.width = 35;
		size.height = 32;
		sPath = "Images/Monsters/Frog_pain.png";
		break;
	case MS_DEATH:
		nSheetmx = 6;
		size.width = 35;
	...
	case MS_JUMPDOWN:
		...
		size.width = 35;
		size.height = 32;
		sPath = "Images/Monsters/Frog_jumpDown.png";
		break;
	...
	if(state == MS_STOP) animation->setDelayPerUnit(0.1f);
	else if (state == MS_LEFT) animation->setDelayPerUnit(0.9f);
	else if (state == MS_RIGHT) animation->setDelayPerUnit(0.9f);
	...
	m_MonsterSprite->setAnchorPoint(Vec2(0.5f, 0.7f));
	m_MonsterSprite->setScale(1.0f);
	layer->addChild(m_MonsterSprite, 3);
...
void Frog::SpriteChange(std::list<Monster>::iterator mob, Layer* layer) {
	...
	Sprite* newSprite = Frog::AnimationState(mob->state, layer);
