...
void MonsterCreator::MonsterPatrol(b2World* world, std::list<Monster> *Monsters, b2Body* Charactor, float movetime, Layer* layer, int foot) {
	...
	for (iter = Monsters->begin(); iter != Monsters->end();) {
		if (iter->obj_name == "Skel") {
			...
			switch (iter->state) {
			case MS_RIGHT:
				iter->body->ApplyLinearImpulse(b2Vec2(0.015, 0), iter->body->GetWorldCenter(), true);
				break;
			case MS_LEFT:
				iter->body->ApplyLinearImpulse(b2Vec2(-0.015, 0), iter->body->GetWorldCenter(), true);
				break;
...
Sprite* MonsterCreator::AnimationState(_moveState state, Layer* layer) {
	int nSheetmx;
	Size size;
	std::string sPath;
	switch (state) {
	...
	case MS_STOP:
		...
		size.width = 31.5;
		size.height = 42;
		sPath = "Images/Monsters/idle.png";
		break;
	case MS_PAIN:
		nSheetmx = 2;
		size.width = 40;
		size.height = 45;
		sPath = "Images/Monsters/pain.png";
		break;
	case MS_DEATH:
		nSheetmx = 6;
		size.width = 38.86;
...
void MonsterCreator::SpriteChange(std::list<Monster>::iterator mob, Layer* layer) {
	...
	Sprite* newSprite = MonsterCreator::AnimationState(mob->state, layer);
